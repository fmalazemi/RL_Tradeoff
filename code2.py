...
def rotate90(L): 
    ...
    # for the outer layer for some NxN network. 

    # That is, L coordinates lay on row/column 0 and N-1 
...
def CalcAvgOfAvg(L, N): 
    ...
    # Record the avg hop count after every fault occurs 
...
def reverseOne(L, N): 
    ...
    # To test if reversing a loop will result in a lower hop count 
...
for i in range(4, 5): 
    ...
    s = str(i) + "x" + str(i) + "\t" + str(calcHopCount(N, L)) + "\t" + str(oneLoopPairs) + "\t" + str( 

        multiLoopPairs) + "\t" + str(avg) + "\t" + str(min_) + "\t" + str(max_) 
